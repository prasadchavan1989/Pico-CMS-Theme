...
Date: '. date('Y/m/d') .'
*/';
		if(file_exists(BLOG_DIR . $file)){
			$error = 'Error: A post already exists with this title';
		} else {
			file_put_contents(BLOG_DIR . $file, $content);
		}
	...
	private function do_open()
	{
		...
		$file .= CONTENT_EXT;
		if(file_exists(BLOG_DIR . $file)) die(file_get_contents(BLOG_DIR . $file));
	...
	private function do_delete()
	{
		...
		$file .= CONTENT_EXT;
		if(file_exists(BLOG_DIR . $file)) die(unlink(BLOG_DIR . $file));
	}
